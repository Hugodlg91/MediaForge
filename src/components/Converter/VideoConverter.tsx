  ...
  const optionsPanel = (
    <>
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <select
          value={outputFormat}
          onChange={(e) => handleFormatChange(e.target.value as VideoFormat)}
      ...
      {showAdvanced && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-gray-900 rounded-xl p-4 border border-gray-800">
          <div className="flex flex-col gap-1.5">
            <label className="text-gray-400 text-xs font-medium">{t("converter.resolution")}</label>
            <select
              value={resolution ?? ""}
              onChange={(e) => setResolution(e.target.value || undefined)}
              className="bg-gray-800 border border-gray-700 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-indigo-500"
            >
              {RESOLUTIONS.map((r) => (
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-gray-400 text-xs font-medium">{t("converter.codec")}</label>
            <select
              value={codec ?? ""}
              onChange={(e) => setCodec(e.target.value || undefined)}
              className="bg-gray-800 border border-gray-700 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-indigo-500"
            >
              {CODECS.map((c) => (
  ...
  return (
    <div className="flex flex-col gap-5 p-6 h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-white text-xl font-semibold">{t("nav.video")}</h2>
